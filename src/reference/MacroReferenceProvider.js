...
export default class LocatorReferenceProvider {
  ...
  provideReferences (doc, position, context, token) {
    ...
    const match = text.match(regexUtil.commandRegexGroup)
    ...
    try {
      const segment = path.resolve(__dirname, `../../metrics/reference/macros/${fileUtil.getFileName(doc.uri.fsPath)}#${match[1]}.json`)
